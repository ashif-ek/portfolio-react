...
const skillsData = [
  { name: 'Django', level: 95, category: 'Technology', color: '#44B78B', icon: <PythonIcon /> },
  { name: 'JavaScript', level: 90, category: 'Technology', color: '#F7DF1E', icon: <JSIcon /> },
  { name: 'React', level: 90, category: 'Technology', color: '#61DAFB', icon: <ReactIcon /> },
  { name: 'Flutter', level: 75, category: 'Technology', color: '#027DFD', icon: <MobileIcon /> },
  { name: 'Agile Dev', level: 95, category: 'Methodology', color: '#F05032', icon: <AgileIcon /> },
  { name: 'Cybersecurity', level: 80, category: 'Methodology', color: '#00A86B', icon: <ShieldIcon /> },
  { name: 'Team Leadership', level: 98, category: 'Leadership', color: '#A855F7', icon: <BriefcaseIcon /> },
  { name: 'Strategic Planning', level: 92, category: 'Leadership', color: '#EC4899', icon: <PencilRulerIcon /> },
];
...
// --- Memoized Reusable UI Components ---
...
const SkillCard = React.memo(({ skill, isVisible }) => {
  ...
  return (
    <div
      style={cardStyle}
      className="bg-neutral-900/70 backdrop-blur-md border border-neutral-800 rounded-2xl p-6 flex flex-col items-center justify-between gap-4 transition-all duration-300 group hover:bg-neutral-900 hover:border-neutral-700 hover:-translate-y-2 hover:shadow-[0_0_var(--glow-blur)_var(--glow-color)]"
    >
      ...
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-400 max-w-3xl mx-auto text-lg md:text-xl">
            A curated overview of my professional capabilities, spanning technology, methodology, and leadership.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-14">
          {skillCategories.map(category => (
            <button
              ...
              className={`px-6 py-2 text-base font-medium rounded-md transition-all duration-300 border ${
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {filteredSkills.map((skill, index) => (
            <div
              ...
              style={{
                transitionDelay: `${isVisible ? index * 120 : 0}ms`,
                ...
                {toolsData.map(tool => (
                    <div key={tool.name} className="group flex flex-col items-center gap-3 text-gray-500 transition-all duration-300 cursor-pointer">
                        <div className="w-16 h-16 bg-neutral-900 border border-neutral-800 rounded-xl flex items-center justify-center transition-all duration-300 group-hover:bg-neutral-800 group-hover:border-neutral-700 group-hover:scale-105">
                           <div className="text-4xl text-gray-400 transition-colors group-hover:text-white">{tool.icon}</div>
                        </div>
